...
class Net(nn.Module):
    def __init__(self, **kwargs):
        ...
        self.thresholds2 = torch.tensor(np.linspace(0.13, 0.98, 15), requires_grad=False).cuda()
        
        self.downscale = nn.AvgPool2d(3, stride=3)
        self.conv1 = nn.Conv2d(6, n_kernels, kernel_size=5, bias = True)
    ...
    def forward(self, x):
        ...
        x = self.downscale(x)
        x1 = sigmoid_quantize(x, self.thresholds1)
        x2 = sigmoid_quantize(x, self.thresholds2)
