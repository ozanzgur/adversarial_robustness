...
def sigmoid_quantize(x, thresholds, multiplier = 10000):
    ...
    for i_ch in range(x.shape[1]):
        ...
        for i, th in enumerate(thresholds):
            output[:, i] = sigmoid(x[:, i_ch, :, :], mean=th, multiplier=multiplier)
...
def sigmoid(x, multiplier, mean = 0):
...
class Net(nn.Module):
    def __init__(self, **kwargs):
        ...
        self.thresholds1 = torch.tensor(np.linspace(0.07, 0.93, 15), requires_grad=False).cuda()
        self.thresholds2 = torch.tensor(np.linspace(0.1, 0.9, 9), requires_grad=False).cuda()
    ...
    def forward(self, x):
        ...
        x = self.downscale(x)
        x = sigmoid_quantize(x, self.thresholds1)
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.activation(x)
        x = sigmoid_quantize(x, self.thresholds2, multiplier=10)
        x = self.avgpool(x)
        x = self.flatten1(x)
        x = self.fc2(x)
        x = self.softmax(x)
