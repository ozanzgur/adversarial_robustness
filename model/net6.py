...
class SEBlock(nn.Module):
    ...
    def forward(self, in_block):
        ...
        x = self.fc2(x)
        x = self.sigmoid(x)
        self.sigmoid_output = x.clone()
        # print(f"sigmoid.shape={x.shape}")
        # print(f"in_block.shape={in_block.shape}")
        return x.unsqueeze(-1).unsqueeze(-1) * in_block
...
class Net(nn.Module):
    def __init__(self, **kwargs):
        ...
        self.se1 = SEBlock(n_channels=n_kernels, ratio=10)
        self.relu1 = nn.ReLU()
        ...
        self.se2 = SEBlock(n_channels=part_size*2, ratio=10)
        self.relu2 = nn.ReLU()
        ...
        self.se3 = SEBlock(n_channels=part_size*2, ratio=10)
        self.relu3 = nn.ReLU()
